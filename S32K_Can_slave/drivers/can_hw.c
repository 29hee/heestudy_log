...
// 주의: 현재 규모에서는 callback 안에서 frame 복사와 queue 적재까지 수행하지만,
// 버스 부하가 커지면 ISR/callback 부담을 더 줄일지 검토할 여지가 있다.
static void CanHw_OnRxComplete(CanHw *hw)
{
    ...
    {
        // callback 문맥에서는 raw frame 저장까지만 수행한다.
    ...
    // 다음 frame을 받기 위한 재무장을 여기서 바로 건다.
...
// 주의: 이 계층은 최근 에러 코드와 간단한 카운터만 유지한다.
// 더 자세한 진단을 보려면 상위에서 이 값을 노출하거나 로그로 올려야 한다.
void CanHw_Task(CanHw *hw, uint32_t now_ms)
{
    ...
    if (hw->tx_busy != 0U)
    {
        // TX mailbox 상태를 polling으로도 확인해 callback 누락 상황을 보완한다.
    ...
    // RX mailbox가 에러 상태라면 다음 수신을 받을 수 있게 다시 걸어 둔다.
...
// 버스 부하가 더 커지면 callback 일을 더 짧게 나누고, drop 통계를 상위까지 끌어올리는 쪽이 안전하다.

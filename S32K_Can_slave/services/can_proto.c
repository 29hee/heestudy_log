...
// decode 목적지를 빈 메시지 상태로 맞춘다.
// 이전 결과가 남아 있으면 frame이 일부만 유효할 때 헷갈리기 쉬워,
// 시작할 때 한 번 깨끗하게 비워 둔다.
...
// 프로젝트가 허용하는 node id 범위인지 확인한다.
// 브로드캐스트는 예외로 받아 주고,
// 그 외 값은 정의한 논리 주소 범위 안에서만 통과시킨다.
...
// text payload가 사람이 읽을 수 있는 범위의 문자로만 이뤄졌는지 본다.
// text 전송은 짧고 단순한 상태 문구를 전제로 하므로,
// 제어 문자나 비정상 바이트는 여기서 걸러 둔다.
...
// 논리 메시지 종류를 실제 CAN ID로 바꾼다.
// encode 경로가 message type 분기와 ID 결정을 한곳에서 공유하게 하려는 보조 함수다.
...
// 수신 frame의 CAN ID를 프로젝트 메시지 종류로 되돌린다.
// 모르는 ID는 무시 대상으로 남기고,
// 아는 ID만 다음 decode 단계로 넘긴다.
...
// 주의: 현재 구현은 command/response/event/text 모두 단일 frame만 지원한다.
// text도 최대 CAN_TEXT_MAX_LEN 범위의 짧은 ASCII 문장만 허용한다.
                               ...
                               CanEncodedFrameList *out_list)
{
    ...
    {
        // text는 별도 header + 본문이 한 frame 안에 들어가는 형식이다.
...
// 주의: 현재 decoder는 표준 ID 기반 단일 frame 포맷만 해석한다.
// extended ID, remote frame, 다중 frame 조립은 지원하지 않는다.
...
// 참고:
// 지금 프로토콜은 1-frame 메시지에 맞춰 단순하게 잡혀 있다.
// payload가 더 길어지거나 버전이 늘어날 계획이 생기면 frame 분할과 호환 규칙을 먼저 정리해 두는 편이 좋다.

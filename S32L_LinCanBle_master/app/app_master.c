...
// NOTE:
// 이 함수는 fail-open 대신 fail-closed 성향으로 작성되어 있다.
// 최신 status가 없거나 stale이면 승인 흐름을 시작하지 않고 먼저 대기/거절로 돌린다.
void AppMaster_RequestOk(AppCore *app, uint32_t now_ms)
{
    ...
    // 1) 승인 판단 전에 최신 LIN status가 실제로 신뢰 가능한지부터 확인한다.
    ...
    // 2) 조건을 만족하면 다음 poll 슬롯용 OK token 요청을 적재한다.
...
// NOTE:
// 이 함수는 현재 정책 판단, UI 문자열 생성, 일부 CAN side-effect까지 함께 맡고 있다.
// 지금 규모에서는 읽기 괜찮지만, 추후에는 policy/presentation 분리 후보로 보는 편이 좋다.
void AppMaster_HandleFreshLinStatus(AppCore *app, const LinStatusFrame *status)
{
    ...
    // 1) 현재 status를 사람이 읽는 값과 내부 기본 상태로 정리한다.
    ...
    // 2) 세부 정책 판단 전에 UI의 기본 status field를 먼저 동기화한다.
    AppMaster_UpdateLinStatusUi(app, status, ui_ok_relay_active, zone_text);

    // 3) fail-closed 기준으로 현재 master mode를 계산한다.
    ...
    // 4) emergency/fault 전이가 실제로 바뀐 경우에만 전이 부작용을 실행한다.
    ...
    // 5) 같은 상태 반복 출력은 줄이고, 의미 있는 LIN 상태 변화만 result에 남긴다.
    ...
    // 6) 진행 중인 OK relay가 있으면 최신 status를 기준으로 승인/취소를 이어서 판단한다.
...
// NOTE:
// 현재 구조는 poll 슬롯 하나를 status/ok가 공유한다.
// 그래서 relay 중에는 단순하고 읽기 쉬운 대신 throughput과 반응성은 다소 제한된다.
void AppMaster_AfterLinPoll(AppCore *app, uint32_t now_ms)
{
    ...
    // 1) 최신 fresh status가 있으면 우선 relay 종료 조건부터 다시 확인한다.
    status_ready = LinModule_GetLatestStatusIfFresh(&app->lin_module,
                                                    now_ms,
                                                    APP_LIN_STATUS_MAX_AGE_MS,
                                                    &status);
    ...
    // 2) fresh status가 없더라도 전체 relay timeout은 별도로 감시한다.
    ...
    // 3) timeout 전에 retry 상한도 별도로 제한한다.
    ...
    // 4) 재시도 간격이 되었으면 다음 poll 슬롯용 OK token을 다시 적재한다.
    request_status = LinModule_RequestOk(&app->lin_module);
    if (request_status == INFRA_STATUS_OK)
    {

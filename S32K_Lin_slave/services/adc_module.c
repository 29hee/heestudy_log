...
// 주의: 샘플 실패 시에는 fail-closed snapshot으로 즉시 전환한다.
// 이 덕분에 상위 계층은 "fault도 위험 상태"라는 기준으로 단순하게 해석할 수 있다.
void AdcModule_Task(AdcModule *module, uint32_t now_ms)
{
    ...
    // 1) 샘플 주기가 아직 안 됐으면 아무 것도 하지 않는다.
    ...
    // 3) 설정 범위를 넘어가는 값은 ceiling으로 clamp한다.
    ...
    // 5) emergency에 한 번 들어가면 승인 전까지 latch를 유지한다.
...
// 이른 복구를 방지한다.
// 주의: token이 들어왔다고 바로 해제하지 않는다.
// empty, fault, 아직 emergency인 경우를 각각 다른 상태 코드로 남겨 상위가 이유를 구분할 수 있게 한다.
